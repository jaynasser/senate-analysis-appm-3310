...
def split_votes_df_by_party(df_votes: pd.DataFrame, df_senators: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
	...
	df_votes_party = df_votes_party.merge(df_votes_party.groupby(['party_code', 'rollnumber'])['cast_code'].apply(lambda x: x.mode().iloc[0]).rename(index='majority_vote'), how='left', left_on=['party_code', 'rollnumber'], right_index=True)
	
	df_votes_party = df_votes_party[df_votes_party['majority_vote'] != 0]
...
def split_votes_df_by_party_old(df_votes, df_senators):
...
def cos_sim(
	...
	saveout_fp: Tuple = ('senate_cos_sim_dem.png', 'senate_cos_sim_rep.png')
) -> None:
...
#	cos_sim(saveout_plot=True)
	...
	df_votes_dem, df_votes_rep = split_votes_df_by_party(df_votes, df_senators)
	df_votes_dem_old, df_votes_rep_old = split_votes_df_by_party_old(df_votes, df_senators)

	print(create_cosine_similarity_matrix(df_votes_rep) - create_cosine_similarity_matrix(df_votes_rep_old)) # dem metrics should differ because of row dropping. no row drops needed for reps.